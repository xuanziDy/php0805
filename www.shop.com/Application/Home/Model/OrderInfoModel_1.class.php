...
//    }
    ...
    public function getOrderInfoList($member_id){
        ...
        //所有订单
        $orders=$this->where($where)->field("id as order_id,sn,pay_status,order_status,shipping_status")->select();
//      dd($orders);
        $map=array();
        if(!empty($orders)){
            $order_ids=arr2str(array_column($orders,'order_id'),',');
            $infos=M('order_info_item')->alias('oii')->join('left join __COMMENT__ AS c ON c.goods_id=oii.goods_id')->where("oii.order_info_id in ($order_ids)")->field('oii.*,c.start,c.content,c.replay,c.status as comment_status')->select();
